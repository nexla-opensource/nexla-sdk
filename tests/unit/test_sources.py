...
class TestSourcesModels:
    ...
    def test_source_model_with_all_fields(self):
        ...
        assert_model_valid(
            source, {"id": source_data["id"], "name": source_data["name"]}
        )
    ...
    def test_source_model_with_credentials(self):
        ...
        source_data = MockResponseBuilder.source(
            source_id=456, include_credentials=True
        )
        source = Source(**source_data)
    ...
    def test_source_model_with_datasets(self):
        ...
        source_data = MockResponseBuilder.source(source_id=789, include_datasets=True)
        source = Source(**source_data)
    ...
    def test_source_copy_options_model(self):
        ...
        options = SourceCopyOptions(
            reuse_data_credentials=True, copy_access_controls=False, owner_id=456
        )
...
class TestSourcesResourceUnit:
    ...
    def test_list_sources_with_filters(self, mock_client, mock_http_client):
        ...
        # Act
        sources = mock_client.sources.list(page=1, per_page=10, access_role="owner")
    ...
    def test_get_source_with_expand(self, mock_client, mock_http_client):
        ...
        mock_source = MockResponseBuilder.source(
            source_id=source_id, include_datasets=True, include_credentials=True
        )
        mock_http_client.add_response(f"/data_sources/{source_id}", mock_source)
        ...
        assert len(source.data_sets) > 0
        mock_http_client.assert_request_made(
            "GET", f"/data_sources/{source_id}", params={"expand": 1}
        )
    ...
    def test_create_source_success(self, mock_client, mock_http_client):
        ...
        create_data = SourceCreate(
            name="New Test Source", source_type="s3", data_credentials_id=789
        )
        mock_response = MockResponseBuilder.source(
            source_id=999, name="New Test Source"
        )
    ...
    def test_update_source_success(self, mock_client, mock_http_client):
        ...
        update_data = SourceUpdate(
            name="Updated Source Name", description="Updated description"
        )
        mock_response = MockResponseBuilder.source(
            source_id=source_id, name="Updated Source Name"
        )
    ...
    def test_delete_source_success(self, mock_client, mock_http_client):
        ...
        mock_http_client.add_response(
            f"/data_sources/{source_id}", {"status": "deleted"}
        )
    ...
    def test_activate_source_success(self, mock_client, mock_http_client):
        ...
        # Arrange
        source_id = 888
        mock_response = MockResponseBuilder.source(source_id=source_id, status="ACTIVE")
        mock_http_client.add_response(
            f"/data_sources/{source_id}/activate", mock_response
        )
        ...
        mock_http_client.assert_request_made(
            "PUT", f"/data_sources/{source_id}/activate"
        )
    ...
    def test_pause_source_success(self, mock_client, mock_http_client):
        ...
        # Arrange
        source_id = 999
        mock_response = MockResponseBuilder.source(source_id=source_id, status="PAUSED")
    ...
    def test_copy_source_success(self, mock_client, mock_http_client):
        ...
        mock_response = MockResponseBuilder.source(source_id=222, name="Copied Source")
        ...
        mock_http_client.assert_request_made(
            "POST",
            f"/data_sources/{source_id}/copy",
            json={"reuse_data_credentials": True, "copy_access_controls": False},
        )
...
class TestSourcesValidation:
    ...
    def test_source_create_validates_enum_fields(self):
        ...
        valid_create = SourceCreate(name="Test", source_type="s3")
    ...
    def test_run_info_model(self):
        ...
        run_data = {"id": 789, "created_at": "2023-01-01T12:00:00Z"}
