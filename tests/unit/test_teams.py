...
class TestTeamsUnitTests:
    ...
    def test_list_teams_with_access_role_member(self, mock_client):
        ...
        client.http_client.assert_request_made(
            "GET", "/teams", params={"access_role": "member"}
        )
    ...
    def test_list_teams_with_pagination(self, mock_client):
        ...
        client.http_client.assert_request_made(
            "GET", "/teams", params={"page": 2, "per_page": 50}
        )
    ...
    def test_get_team_not_found(self, mock_client):
        ...
        client.http_client.add_error(
            "/teams/999",
            HttpClientError(
                "Not found", status_code=404, response={"message": "Team not found"}
            ),
        )
    ...
    def test_create_team_success(self, mock_client):
        ...
        request_data = TeamCreate(
            ...
            members=[TeamMemberRequest(email="test@example.com", admin=True)],
        )
        ...
        response_data.update(
            {
                ...
                "name": "Test Team",
                "description": "A test team",
                "members": [{"id": 456, "email": "test@example.com", "admin": True}],
            }
        )
    ...
    def test_add_members_success(self, mock_client):
        ...
        response_data = [
            MockResponseBuilder.team_member(
                user_id=456, email="existing@example.com", admin=True
            ),
            MockResponseBuilder.team_member(
                user_id=789, email="new1@example.com", admin=True
            ),
            MockResponseBuilder.team_member(
                user_id=790, email="new2@example.com", admin=False
            ),
        ]
    ...
    def test_replace_members_success(self, mock_client):
        ...
        response_data = [
            MockResponseBuilder.team_member(
                user_id=999, email="only@example.com", admin=True
            )
        ]
        client.http_client.add_response("/teams/123/members", response_data)
    ...
    def test_remove_members_success(self, mock_client):
        ...
        response_data = [
            MockResponseBuilder.team_member(
                user_id=456, email="remaining@example.com", admin=False
            )
        ]
        client.http_client.add_response("/teams/123/members", response_data)
    ...
    def test_team_with_complex_members(self, mock_client):
        ...
        team_data = MockResponseBuilder.team(
            ...
            members=[
                MockResponseBuilder.team_member(
                    user_id=1, email="admin@example.com", admin=True
                ),
                MockResponseBuilder.team_member(
                    user_id=2, email="user@example.com", admin=False
                ),
                MockResponseBuilder.team_member(
                    user_id=3, email="another@example.com", admin=True
                ),
            ],
        )
        client.http_client.add_response("/teams/123", team_data)
    ...
    def test_update_team_partial_data(self, mock_client):
        ...
        response_data = MockResponseBuilder.team(
            team_id=123, description="New description only"
        )
        client.http_client.add_response("/teams/123", response_data)
