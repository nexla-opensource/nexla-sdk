...
def test_service_key_obtain_and_ensure_token():
    ...
    mock_http.add_response(
        "/token", create_auth_token_response(access_token="tk-1", expires_in=60)
    )

    auth = TokenAuthHandler(
        service_key="sk-123",
        base_url="https://api.test/nexla-api",
...
def test_execute_authenticated_request_retries_on_401_with_service_key():
    ...
    mock_http.add_response(
        "/token", create_auth_token_response(access_token="tk-0", expires_in=3600)
    )
        ...
        if "/widgets" in req["url"]:
            ...
            if attempt["n"] == 1:
                ...
                raise HttpClientError(
                    "unauthorized", status_code=401, response={"error": "unauthorized"}
                )
            return {"status": "ok"}
    ...
    out = auth.execute_authenticated_request(
        "GET", "https://api.test/nexla-api/widgets", headers={}
    )
    ...
    posts = [
        r for r in mock_http.requests if r["method"] == "POST" and "/token" in r["url"]
    ]
    assert len(posts) >= 1
...
def test_logout_clears_token_and_calls_endpoint():
    mock_http = MockHTTPClient()
    mock_http.add_response(
        "/token", create_auth_token_response(access_token="tk", expires_in=3600)
    )
    mock_http.add_response("/token/logout", {"status": "ok"})
    auth = TokenAuthHandler(
        service_key="sk-1", base_url="https://api.test/nexla-api", http_client=mock_http
    )
