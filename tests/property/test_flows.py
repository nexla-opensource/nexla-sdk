...
class TestFlowsProperty:
    ...
    def test_flow_node_structure_invariants(self, depth, children_per_node):
        ...
        def create_node_with_children(
            parent_id: Optional[int], current_depth: int
        ) -> Dict[str, Any]:
            ...
            node = {
                ...
                "data_source": (
                    {"id": factory.fake.random_int(1, 10000)}
            ...
            if current_depth < depth:
                ...
                node["data_sinks"] = [
                    factory.fake.random_int(1, 10000)
                    for _ in range(factory.fake.random_int(0, 2))
                ]
    ...
    def _validate_node_invariants(
        self, node: Dict[str, Any], parent_id: Optional[int] = None
    ) -> None:
    ...
    def test_flow_response_parsing(self, include_elements, num_flows, include_metrics):
            ...
            mock_response.update(
                {
                    ...
                    "data_credentials": [
                        factory.create_mock_credential() for _ in range(1)
                    ],
                }
            )
        ...
        if include_metrics:
            mock_response["metrics"] = [
                factory.create_mock_flow_metrics() for _ in range(3)
            ]
    ...
    def test_flow_copy_options_validation(
        self, reuse_credentials, copy_access, copy_dependent, owner_id, org_id
    ):
    ...
    def test_flow_api_parameter_combinations(self, data):
        ...
        mock_response = factory.create_mock_flow_response(
            include_elements=not flows_only
        )
        if include_metrics:
