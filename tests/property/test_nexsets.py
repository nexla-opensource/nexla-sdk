...
class TestNexsetsProperty:
    ...
    @SETTINGS
    def test_create_nexset_serialization(
        self, client, nexset_name, nexset_description, parent_id
    ):
        ...
        response_data=st.fixed_dictionaries(
            {
                ...
                "status": st.sampled_from(
                    ["ACTIVE", "PAUSED", "DRAFT", "PROCESSING", "ERROR"]
                ),
            }
        )
    )
    ...
    @given(
        update_name=st.one_of(
            st.none(), st.text(min_size=1, max_size=100).filter(lambda x: x.strip())
        ),
    ...
    @SETTINGS
    def test_update_nexset_with_various_data(
        self, client, update_name, update_description, tags
    ):
    ...
    def test_nexset_samples_with_various_data(self, client, assertions, sample_data):
        ...
        # Arrange
        nexset_id = 1001
        mock_sample = MockResponseBuilder.nexset_sample({"raw_message": sample_data})
    ...
    def test_get_samples_parameters(self, client, count, include_metadata, live):
        ...
        nexset_id = 1001
        mock_response = [
            MockResponseBuilder.nexset_sample() for _ in range(min(count, 5))
        ]
        client.http_client.request = MagicMock(return_value=mock_response)

        # Act
        client.nexsets.get_samples(
            nexset_id, count=count, include_metadata=include_metadata, live=live
        )
    ...
    @given(
        ...
        flow_type=st.one_of(
            st.none(), st.sampled_from(["batch", "streaming", "real_time"])
        ),
        status=st.sampled_from(["ACTIVE", "PAUSED", "DRAFT", "PROCESSING"]),
    )
    @SETTINGS
    def test_nexset_name_and_type_combinations(
        self, client, assertions, nexset_name, flow_type, status
    ):
        """Test nexsets with various name and type combinations."""
        # Arrange
        mock_response = MockResponseBuilder.nexset(
            {"name": nexset_name.strip(), "flow_type": flow_type, "status": status}
        )
    ...
    @SETTINGS
    def test_copy_options_serialization(
        self, client, copy_access_controls, owner_id, org_id
    ):
        ...
        copy_options = NexsetCopyOptions(
            copy_access_controls=copy_access_controls, owner_id=owner_id, org_id=org_id
        )
