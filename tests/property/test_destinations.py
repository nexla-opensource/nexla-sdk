...
class TestDestinationsProperty:
    ...
    @SETTINGS
    def test_create_destination_serialization(
        self, client, destination_name, destination_description, sink_type
    ):
        ...
        response_data=st.fixed_dictionaries(
            {
                ...
                "sink_type": st.sampled_from(
                    ["s3", "gcs", "mysql", "postgres", "snowflake", "bigquery", "kafka"]
                ),
    ...
    @SETTINGS
    def test_list_destinations_response_parsing(
        self, client, assertions, destinations_data
    ):
        """Test parsing list destinations responses with various data combinations."""
        # Arrange
        mock_destinations = [
            MockResponseBuilder.destination(data) for data in destinations_data
        ]
    ...
    @given(
        name=st.one_of(
            st.none(), st.text(min_size=1, max_size=200).filter(lambda x: x.strip())
        ),
        description=st.one_of(st.none(), st.text(max_size=500)),
        data_credentials_id=st.one_of(
            st.none(), st.integers(min_value=1, max_value=999999)
        ),
        data_set_id=st.one_of(st.none(), st.integers(min_value=1, max_value=999999)),
    )
    @SETTINGS
    def test_update_destination_with_various_data(
        self, client, assertions, name, description, data_credentials_id, data_set_id
    ):
    ...
    @SETTINGS
    def test_destination_with_various_sink_configs(
        self, client, assertions, sink_config_data
    ):
        """Test destinations with various sink configuration combinations."""
        # Arrange
        mock_response = MockResponseBuilder.destination(
            {"id": 12345, "sink_config": sink_config_data}
        )
        client.http_client.request = MagicMock(return_value=mock_response)
        ...
        if hasattr(destination, "sink_config") and destination.sink_config:
            assert (
                destination.sink_config["data_format"]
                == sink_config_data["data_format"]
            )
            assert (
                destination.sink_config["mapping"]["mode"]
                == sink_config_data["mapping"]["mode"]
            )
    ...
    @SETTINGS
    def test_destination_name_edge_cases(
        self, client, assertions, destination_name, sink_type
    ):
        ...
        mock_response = MockResponseBuilder.destination(
            {"name": destination_name.strip(), "sink_type": sink_type}
        )
        client.http_client.request = MagicMock(return_value=mock_response)
    ...
    @SETTINGS
    def test_destination_with_nested_objects(
        self, client, assertions, data_set_info, data_map_info
    ):
        """Test destinations with various nested object combinations."""
        # Arrange
        mock_response = MockResponseBuilder.destination(
            {"id": 12345, "data_set": data_set_info, "data_map": data_map_info}
        )
        client.http_client.request = MagicMock(return_value=mock_response)
    ...
    @SETTINGS
    def test_destination_vendor_configuration(
        self, client, assertions, vendor_data, vendor_endpoint
    ):
