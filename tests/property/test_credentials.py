...
def credential_dict(draw):
    ...
    return {
        ...
        "verified_status": draw(
            st.one_of(st.none(), st.sampled_from(["VERIFIED", "UNVERIFIED", "FAILED"]))
        ),
        "credentials_version": draw(
        ...
        "created_at": draw(
            st.one_of(st.none(), st.datetimes().map(lambda dt: dt.isoformat() + "Z"))
        ),
        "updated_at": draw(
            st.one_of(st.none(), st.datetimes().map(lambda dt: dt.isoformat() + "Z"))
        ),
...
def credential_create_dict(draw):
    ...
    credentials_type = draw(
        st.sampled_from(["s3", "postgres", "mysql", "rest", "bigquery"])
    )
...
class TestCredentialModelProperties:
    ...
    def test_credential_name_property(self, name):
        ...
        minimal_data = {"id": 1, "name": name, "credentials_type": "s3"}
...
class TestCredentialEdgeCases:
    ...
    def test_credential_with_empty_optional_fields(self):
        ...
        minimal_data = {"id": 1, "name": "Minimal Credential", "credentials_type": "s3"}
    ...
    def test_credential_with_various_list_sizes(self, int_list):
        ...
        access_roles = [
            valid_roles[i % len(valid_roles)] for i in int_list[:10]
        ]  # Limit size
