...
class TestUsersPropertyTests:
    ...
    def test_user_creation_with_various_inputs(
        self,
        mock_client,
        user_id,
        email,
    ...
    def test_user_response_parsing_with_org_memberships(
        self, mock_client, org_memberships
    ):
        """Test user response parsing with various org membership configurations."""
        client = mock_client

        user_data = MockResponseBuilder.user(
            user_id=123, org_memberships=org_memberships
        )
        ...
        for i, membership in enumerate(user.org_memberships):
            ...
            assert (
                membership.org_membership_status
                == org_memberships[i]["org_membership_status"]
            )
    ...
    @given(
        ...
        access_role=st.one_of(
            st.none(),
            st.sampled_from(["owner", "collaborator", "operator", "admin", "all"]),
        ),
    )
    def test_list_users_with_various_parameters(
        self, mock_client, users_count, page, per_page, access_role
    ):
        ...
        users_data = [
            MockResponseBuilder.user(user_id=i + 1) for i in range(users_count)
        ]
    ...
    def test_user_update_with_various_fields(
        self, mock_client, name, email, status, user_tier, password
    ):
    ...
    def test_account_metrics_with_various_parameters(
        self, mock_client, from_date, to_date, org_id
    ):
        ...
        metrics_data = {"total_sources": 5, "total_sinks": 3, "total_records": 10000}

        client.http_client.add_response(
            "GET", "/users/123/flows/account_metrics", metrics_data
        )
    ...
    def test_daily_metrics_with_various_parameters(
        self, mock_client, resource_type, from_date, to_date, org_id
    ):
        ...
        metrics_data = {"daily_records": 1000, "resource_type": resource_type}

        client.http_client.add_response("GET", "/users/123/metrics", metrics_data)

        kwargs = {"resource_type": resource_type, "from_date": from_date}
        if to_date is not None:
            kwargs["to_date"] = to_date
        if org_id is not None:
            kwargs["org_id"] = org_id
    ...
    def test_user_creation_request_validation(
        self, full_name, email, default_org_id, status, user_tier
    ):
        """Test user creation request with various input combinations."""
        # Create request with only non-None values
        create_data = {"full_name": full_name, "email": email}
    ...
    def test_list_users_parameter_combinations(
