...
class TestSourcesIntegration:
    ...
    def test_create_update_delete_source_lifecycle(self, integration_client):
        ...
        try:
            ...
            # Act - Update
            update_data = SourceUpdate(description="Updated integration test source")
            updated_source = integration_client.sources.update(
                created_source.id, update_data
            )
            ...
            if created_source.status in ["INIT", "PAUSED"]:
                activated_source = integration_client.sources.activate(
                    created_source.id
                )
            ...
            try:
                ...
                print(
                    f"Warning: Failed to clean up test source {created_source.id}: {e}"
                )
