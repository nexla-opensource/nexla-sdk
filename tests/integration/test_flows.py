...
class TestFlowsIntegration:
    ...
    def test_get_flow_by_source(self, client, test_source_id):
        ...
        if flow.flows:
            ...
            source_found = any(
                node.data_source_id == test_source_id for node in flow.flows
            )
    ...
    def _validate_flow_node(self, node: FlowNode) -> None:
        ...
        if node.parent_node_id is None:
            assert node.data_source_id is not None or (
                hasattr(node, "data_source") and node.data_source is not None
            )
