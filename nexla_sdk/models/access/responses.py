...
class UserAccessorResponse(BaseModel):
    ...
    org_id: Optional[int] = Field(
        None, description="Organization ID for cross-org access"
    )
...
class OrgAccessorResponse(BaseModel):
    ...
    client_identifier: Optional[str] = Field(
        None, description="Client identifier for the organization"
    )
    email_domain: Optional[str] = Field(
        None, description="Email domain for the organization"
    )
    access_roles: List[AccessRole] = Field(description="List of access roles")
    created_at: Optional[datetime] = Field(None, description="Creation timestamp")
    updated_at: Optional[datetime] = Field(None, description="Last update timestamp")
